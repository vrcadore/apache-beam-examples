...
def tokenize_sentence(
    text_and_mask: Tuple[str, str], bert_tokenizer: BertTokenizer
...
def parse_known_args(argv):
    ...
    parser.add_argument(
        "--input", dest="input", help="Path to the text file containing sentences."
    )
    parser.add_argument(
        "--output",
        dest="output",
...
def run(
    ...
    text_and_tokenized_text_tuple = (
        ...
        | "TokenizeSentence" >> beam.Map(lambda x: tokenize_sentence(x, bert_tokenizer))
    )
    output = (
        text_and_tokenized_text_tuple
