...
MODELS = [
    ...
    {"name": "floor_area", "required_features": ["Area", "Year", "TotalFloorArea"]},
    {
        "name": "stations",
        "required_features": [
            "Area",
...
def run(argv=None, save_main_session=True, test_pipeline=None) -> PipelineResult:
    ...
    [
        ...
        stations,
        no_features,
    ] = loaded_data | "Partition" >> beam.Partition(sort_by_features, len(MODELS))
    ...
    prediction_report = flattened_predictions | "AllPredictions" >> beam.Map(
        report_predictions
    )
